...
import { Bell, Menu, ChevronDown, LogOut, Settings, User, Building2 } from 'lucide-react';
...
const PAGE_TITLES = {
  "/": {
    title: "Customers",
    subtitle: "Manage all service work items.",
  },

  "/accessories": {
    title: "Accessories",
    subtitle: "Track products bought per work type.",
  },

  "/amount": {
    title: "Amount Summary",
    subtitle: "Overall and per-customer financials.",
  },
...
export default function TopNav({ onMenuClick, notifCount = 0 }) {
  ...
  const info =
    ...
    PAGE_TITLES["/"];
  const { user, company, logout, profile } = useAuth();
                ...
                <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-2xl shadow-modal border border-gray-100 z-20 overflow-hidden animate-scale-in">
                  ...
                  <div className="p-1.5">
                    <button
                      ...
                      className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-gray-600 rounded-xl hover:bg-gray-50"
                    >
                    ...
                    <button onClick={() => { setProfileOpen(false); navigate("/settings"); }}
                      className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-gray-600 rounded-xl hover:bg-gray-50 transition-colors">
                      <Settings className="w-4 h-4" /> Settings
                    </button>
                    <button onClick={handleLogout} className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-red-500 rounded-xl hover:bg-red-50 transition-colors">
